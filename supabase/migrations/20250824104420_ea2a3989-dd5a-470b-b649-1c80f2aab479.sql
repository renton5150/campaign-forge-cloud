-- Corriger la fonction queue_campaign_for_sending pour gérer les doublons intelligemment
...
BEGIN
  ...
  -- Réactiver les emails échoués existants pour cette campagne
  UPDATE public.email_queue 
  SET status = 'pending', 
      retry_count = 0,
      error_message = NULL,
      scheduled_for = COALESCE(v_campaign.scheduled_at, now()),
      updated_at = now()
  WHERE campaign_id = p_campaign_id 
    AND status IN ('failed', 'sent')  -- Réactiver les échoués et permettre renvoi
  ...
  RAISE NOTICE '[queue_campaign] Réactivé % emails existants', v_updated_count;

  -- Boucle sur les contacts des listes pour nouveaux emails
  FOR v_contact IN 
    SELECT DISTINCT c.*
    FROM public.contacts c
    JOIN public.contact_list_memberships clm ON c.id = clm.contact_id
  ...
  LOOP
    ...
    -- Vérifier si email existe déjà pour cette campagne
    IF EXISTS (
      ...
      CONTINUE;  -- Skip, sera traité par l'UPDATE ci-dessus
    END IF;
  ...
  -- Message final incluant les emails réactivés
  v_message := (v_queued_count + v_updated_count) || ' emails prêts à être envoyés';
  IF v_queued_count > 0 THEN
    v_message := v_message || ' (' || v_queued_count || ' nouveaux)';
  END IF;
  IF v_updated_count > 0 THEN
    v_message := v_message || ' (' || v_updated_count || ' réactivés)';
  END IF;
  ...
  RAISE NOTICE '[queue_campaign] Done. new=%, reactivated=%, duplicates=%, blacklisted=%', v_queued_count, v_updated_count, v_duplicate_count, v_blacklist_count;

  RETURN jsonb_build_object(
    'success', true,
    'queued_emails', v_queued_count + v_updated_count,
    'new_emails', v_queued_count,
    'reactivated_emails', v_updated_count,
