-- Fix the queue_campaign_for_sending function to handle tenant_id issues and simplify
CREATE OR REPLACE FUNCTION public.queue_campaign_for_sending(p_campaign_id uuid, p_contact_list_ids uuid[] DEFAULT NULL)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
BEGIN
  -- Récupérer la campagne avec informations détaillées
  ...
  IF p_contact_list_ids IS NULL OR array_length(p_contact_list_ids, 1) IS NULL OR array_length(p_contact_list_ids, 1) = 0 THEN
    ...
    -- Si toujours aucune liste trouvée
  ...
  -- Créer une table temporaire pour les emails blacklistés
  CREATE TEMP TABLE IF NOT EXISTS temp_blacklist AS
  SELECT DISTINCT value as email
  FROM public.blacklists
  WHERE (tenant_id = v_campaign.tenant_id OR v_campaign.tenant_id IS NULL)
    AND type = 'email';
